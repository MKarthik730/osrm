...
ROAD_SAFETY_DB = {
    "primary": {"flood_prone": False, "has_streetlight": True, "avg_speed_kmh": 50},
    "secondary": {"flood_prone": False, "has_streetlight": True, "avg_speed_kmh": 40},
    "tertiary": {"flood_prone": False, "has_streetlight": True, "avg_speed_kmh": 30},
    "residential": {"flood_prone": True, "has_streetlight": False, "avg_speed_kmh": 20},
    "unclassified": {
        "flood_prone": True,
        "has_streetlight": False,
        "avg_speed_kmh": 15,
    },
    "motorway": {"flood_prone": False, "has_streetlight": True, "avg_speed_kmh": 80},
}


def get_road_safety(highway_type: str) -> dict:
    """Get road safety metadata based on OSM highway type."""
    return ROAD_SAFETY_DB.get(
        highway_type.lower(),
        {
            "flood_prone": True,
            "has_streetlight": False,
...
async def analyze_route(data: RouteAnalyzeRequest):
    """
    Analyze route for safety flags and segment-by-segment breakdown.
    Returns flagged segments with warnings.
    """
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            url = (
                f"{OSRM_URL}/route/v1/driving/"
                f"{data.from_lon},{data.from_lat};{data.to_lon},{data.to_lat}"
        ...
        for leg in route.get("legs", []):
            for step in leg.get("steps", []):
                ...
                highway = step.get("mode", "unclassified")
                duration = step.get("duration", 0)
                distance = step.get("distance", 0)

                safety = get_road_safety(highway)
                is_flagged = (
                    safety["flood_prone"]
                    or not safety["has_streetlight"]
                    and data.time_of_day >= 18
                    or data.time_of_day <= 6
                )
                ...
                if safety["flood_prone"]:
                    segment_data["warning"] = "Flood-prone area - exercise caution"
                    warnings.append(
                        {
                            "segment": road_name,
                ...
                if not safety["has_streetlight"] and (
                    data.time_of_day >= 19 or data.time_of_day <= 6
                ):
                    segment_data["warning"] = "Poor lighting at night"
                    warnings.append(
                        {
                            "segment": road_name,
                            "reason": "No streetlights",
                            "location": maneuver.get("location", []),
                        }
                    )

                flagged_segments.append(segment_data)
        ...
        return {
            ...
            "safety_score": max(0.0, 1.0 - (len(warnings) * 0.1)),
        }
